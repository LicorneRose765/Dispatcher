 ...
 */
request_t read_request(char *block) {
    // TODO : mutex & semaphore
    request_t request;
    // Copy the block's content into the request
    memcpy(&request, block, sizeof(request_t));
    return request;
}
 ...
 * @return
 */
int write_request(request_t *request, char *block) {
    // TODO : mutex & semaphore
    memcpy(block, &request, sizeof(request_t));
    return 0;
}